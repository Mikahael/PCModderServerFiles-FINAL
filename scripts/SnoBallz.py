...
#ShieldBall based on this snoBall
import bs
import bsUtils
import bsVector
import bsBomb
...
class snoBall(bs.Actor):
    ...
    def __init__(self, position=(0,1,0), velocity=(5,0,5), sourcePlayer=None, owner=None, explode=False):
        ...
        #bs.animate(self.nodeShield,'radius',{0:0.9,200:1,400:0.4},True)
    ...
    def giveBallz(self,spaz):
        ...
        if self._powerExpire:
            ...
            def text():
                if spaz.isAlive():
                    #node = bs.getCollisionInfo("opposingNode")
                    bsUtils.PopupText("Snowballs turned Off!",color=(1,1,1),scale=1.0,position=spaz.node.position).autoRetain() 
            bs.gameTimer(self._powerLife, bs.Call(text))
