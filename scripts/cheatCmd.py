...
class cheatOptions(object):
    ...
    def checkOwner(self,clientID): 
        ...
        if client in MID.owners or client in gph.ownerHashes or client in MID.owner2: 
  ...
  #defend your self from dhokebaaz admins......  
                ...
                #this is the new system for configuration commands.
                    ...
                    if self.checkMod(nick):
                        if fire.vanilla == False and fire.modded_powerups == False:
                            fire.vanilla = True
                            vanillaTrue()
                        else:
                            fire.vanilla = False
                    ...
                    if self.checkMod(nick):
                        if fire.vanilla == False and fire.modded_powerups == False:
                            ...
                            pcpowerupsFalse()
                        k = fire.modded_powerups
                        bs.screenMessage('PC Pwps turned ---> '+str(k))
                elif m == '?discolight': 
                    if self.checkMod(nick):
                        if fire.discoLights == True:
                    ...
                    if self.checkMod(nick):
                     bs.screenMessage('?flash, ?floater, ?spikeModel, ?white, ?name, ?pvt, ?shieldf, ?glovef', clients=[clientID], transient=True)
                     #bs.screenMessage('All rights to PCModder', clients=[clientID], transient=True)
...
def impactbombTrue():
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'r')
    list_of_lines = a_file.readlines()
    list_of_lines[230] = "impact_bomb = True\n"
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'w')
    a_file.writelines(list_of_lines)
    a_file.close()  
def impactbombFalse():
...
def vanillaTrue():
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'r')
    list_of_lines = a_file.readlines()
    list_of_lines[265] = "vanilla = True\n"
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'w')
    a_file.writelines(list_of_lines)
    a_file.close()  
def vanillaFalse():
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'r')
    list_of_lines = a_file.readlines()
    list_of_lines[265] = "vanilla = False\n"
    a_file = open(bs.getEnvironment()['systemScriptsDirectory'] + "/fire.py",'w')
    a_file.writelines(list_of_lines)
    a_file.close()
def pwpboxTrue():
