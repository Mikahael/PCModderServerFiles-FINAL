...
def checkAnswer(msg, clientID):
    ...
    if True:#msg.lower() == correctAnswer:
        if answeredBy is not None:
            bs.screenMessage('Already awarded to ' + answeredBy, color=(1,1,1), clients=[clientID], transient=True)
        else:
            for i in bsInternal._getForegroundHostActivity().players:
                if i.getInputDevice().getClientID() == clientID:
                    answeredBy = i.getName()
                    accountID = i.get_account_id()
		    bs.screenMessage(answeredBy + ': ' + msg, color=(1,1,1))
            try:
                bs.screenMessage('Congratulations ' + answeredBy + '! You won ' + bs.getSpecialChar('ticket') + '10.',color=(1,1,1), clients=[clientID], transient=True)
