...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        def _log_authentication(account):
            import datetime
        ...
        def _add_profile_verification(account, add):
            updated_admins=[]
            updated_admins=MID.verification
            #     
            if add:
                updated_admins.append(account)
            else:
                updated_admins.remove(account)
            with open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py") as file:
                s = [row for row in file]              
                s[47] = 'verification = '+ str(updated_admins)+ '\n'
                f = open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py",'w')
                for i in s:
                    f.write(i)
                f.close()
         ...
         if profile in MID.authentication:
            ...
            if value in account.values():
                if len(matching_values) > 1:
                    ...
                    _log_authentication(account=profile)
        ...
        if profile not in MID.members_count:
            updated_admins=[]
            updated_admins=MID.members_count
            #     
            updated_admins.append(profile)
            with open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py") as file:
                s = [row for row in file]              
                s[48] = 'members_count = '+ str(updated_admins)+ '\n'
                f = open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py",'w')
                for i in s:
                    f.write(i)
                f.close()
